  ...
  <div class="container">
    <transition-grow>
      ...
      <div v-else>
        <form @submit.prevent="doSearch">
          ...
          <div class="relative w-full">
            ...
            <input
              ...
              v-model="search"
              type="text"
              :disabled="isLoading || isErrorState"
              class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-9 p-2.5 pr-32 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400  placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
          ...
          <transition-grow>
            <div v-if="isAmbiguousQuery" class="mt-2 text-orange-700 font-light relative pl-6">
              ...
              <span class="text-sm">
                Unlike Google, this search engine works better by asking actual questions in free form.
                Please end your question with &laquo;?&raquo; or click again on Search button to continue.
              </span>
            </div>
          </transition-grow>
        </form>
        ...
        <transition-grow>
          <div v-if="isAdvancedVisible" class="grid grid-cols-6 gap-x-6">
            <div class="mt-2 col-span-6 sm:col-span-3">
              ...
              <select id="modelName" v-model="modelName" class="block w-full text-xs rounded-md border-0 py-1.5 pl-2 pr-5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-indigo-600">
            ...
            <div class="mt-2 col-span-6 sm:col-span-3">
              ...
              <select id="summaryModelName" v-model="summaryModelName" class="block w-full text-xs rounded-md border-0 py-1.5 pl-2 pr-5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-indigo-600">
                <option v-for="model in summaryModels" :key="model" :value="model">
                  {{ model }}
                </option>
              </select>
            </div>
        ...
        <transition-grow>
          <div v-if="!isLoading && prevSearch">
            <div class="my-6 flex items-center">
              <h2 class="text-xl font-medium text-slate-600 grow">
                {{ prevSearch }}
              </h2>
              ...
              <div class="shrink-0 ml-2">
                ...
                <button
                  ...
                  type="button"
                  class="text-sm flex flex-nowrap items-center rounded-md border-0 py-1.5 pl-3 pr-5 text-slate-700 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-primary-600 bg-slate-200 hover:bg-slate-300"
                  @click="share"
                >
          ...
          <div v-if="!isLoading && id" class="my-4 h-max min-h-[100px] relative">
            <transition-fade>
              ...
              <div v-else role="status" class="absolute inset-0">
                <div class="animate-pulse h-2.5 bg-gray-200 rounded-sm dark:bg-gray-700 w-1/3 mb-4" />
                <div class="animate-pulse h-2 bg-gray-200 rounded-sm dark:bg-gray-700 w-4/5 mb-2.5" />
                <div class="animate-pulse h-2 bg-gray-200 rounded-sm dark:bg-gray-700 w-3/4 mb-2.5" />
                <span class="sr-only">Loading...</span>
                <div class="text-slate-400 text-xs mt-4">
                  Waiting for GPT to summarize the answer from relevant transcripts...
                </div>
              </div>
            </transition-fade>
          </div>
        ...
        <transition-grow>
          <h2 v-if="!isLoading && prevSearch && results?.length" class="text-xl font-medium text-slate-600 pt-2">
...
async function doSearch() {
  ...
  try {
    window.plausible('Search', { props: { query: search.value } });
  } catch (e) { /* do nothing */ }
