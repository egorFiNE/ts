...
async function fillTextsGaps({ libraryId, documentId, wordCount, modelName, texts, ends, sequences, sequelize }) {
...
async function fillSequenceGaps({ searchResults, libraryId, modelName, sequelize }) {
  // TODO rewrite async loop
  for (const entry of searchResults) {
    await fillTextsGaps({ ...entry, libraryId, modelName, sequelize });
  }
}

function joinSequenceGaps({ rawSearchResults, wordCount, documentId }) {
  const context = rawSearchResults.filter(e => e.wordCount === wordCount && e.documentId === documentId);
...
export async function search(configuration, libraryId, embedding, modelName) {
  const sequelize = await init();

  const limit = 200;
  const rawSearchResults = await sequelize.models.Chunk.search({ libraryId, embedding, modelName, limit });

  const uniqueHashOfDocumentAndWordCount = { };

  for (const entry of rawSearchResults) {
    uniqueHashOfDocumentAndWordCount[entry.documentId] ||= new Set();
    uniqueHashOfDocumentAndWordCount[entry.documentId].add(entry.wordCount);
  }

  const overallSearchResults = [];

  for (const documentId of Object.keys(uniqueHashOfDocumentAndWordCount)) {
    const searchResultsByWordCount = {};

    for (const wordCount of Array.from(uniqueHashOfDocumentAndWordCount[documentId].values())) {
      ...
      await fillSequenceGaps({ searchResults, libraryId, modelName, sequelize });
